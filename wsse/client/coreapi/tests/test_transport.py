...
class TestWSSEAuthenticatedHTTPTransport(StaticLiveServerTestCase):
    ...
    def makeClient(self, username, password, *args, **kwargs):
        ...
        """
        wsse_transport = WSSEAuthenticatedHTTPTransport(username, password, *args, **kwargs)
    ...
    def test_auth(self):
        ...
        """
        try:
            _response = self.client.get(self.base_url)
    ...
    def test_auth_reuse(self):
        ...
        """
        try:
            _response_a = self.client.get(self.base_url)
            _response_b = self.client.get(self.base_url)
        except ErrorMessage as e:
            self.fail('Client.get failed with %s' % (str(e),))

    def test_auth_incorrect_password(self):
        """
        Authenticate with an incorrect password. The authentication should fail.
        """
        client = self.makeClient('username', '!' + self.user_secret.secret)

        with self.assertRaises(ErrorMessage) as e:
            _response = client.get(self.base_url)

            self.assertEqual(e.exception.error.title, 'Unauthorized')

    def test_auth_nonexistent_username(self):
        """
        Authenticate with a nonexistent user. The authentication should fail.
        """
        client = self.makeClient('non_existent_user', self.user_secret.secret)
